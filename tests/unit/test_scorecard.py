...
def test_scorecard_to_dict_is_deterministic() -> None:
    ...
    assert a["accuracy"]["per_family"]["ml"]["precision"] == pytest.approx(
        0.3333333333333333
    )
...
def test_append_history_preserves_prior_entries(tmp_path) -> None:
    ...
    assert merged["history"][0]["accuracy_summary"]["recall"] == pytest.approx(
        0.3333333333333333
    )
...
def test_main_check_passes_against_self(tmp_path, monkeypatch) -> None:
    ...
    assert rc == 0
    assert out.exists()
    assert json.loads(out.read_text())["accuracy"]["recall"] == pytest.approx(
        0.3333333333333333
    )
...
def test_main_check_fails_on_regression(tmp_path, monkeypatch) -> None:
    ...
    strong = scorecard_to_dict(
        build_scorecard(_vr(recall=1.0), _br(), git_sha="g", timestamp="t")
    )
    baseline = tmp_path / "baseline.json"
    ...
    monkeypatch.setattr(
        mod, "_run_validation_for_scorecard", lambda: _vr(recall=0.5)
    )
    monkeypatch.setattr(mod, "_run_benchmark_for_scorecard", lambda: _br())
    monkeypatch.setattr(mod, "resolve_git_sha", lambda: "g")
    monkeypatch.setattr(mod, "_now_iso", lambda: "t")
