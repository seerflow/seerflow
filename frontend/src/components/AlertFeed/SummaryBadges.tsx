...
export function SummaryBadges({ counts, status }: { counts: Counts; status: WsStatus }): JSX.Element {
  ...
  return (
    <div className="flex items-center gap-3 border-b px-3 py-2 text-sm">
      <span role="status" aria-live="polite" className="flex items-center gap-1"><span className={cn("inline-block h-2 w-2 rounded-full", dot)} aria-hidden /> <span className="text-muted-foreground">{status}</span></span>
