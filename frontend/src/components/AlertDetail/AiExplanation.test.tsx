...
describe("AiExplanation", () => {
  ...
  it("renders the generatedBy footer when provided", () => {
    render(
      <AiExplanation
        narrative="Narrative"
        provenance={[]}
        generatedBy="sf-llm · explain v2"
      />,
    );
    expect(screen.getByText(/generated by sf-llm · explain v2/i)).toBeInTheDocument();
  });

  it("omits the generatedBy footer when not provided", () => {
    render(<AiExplanation narrative="Narrative" provenance={[]} />);
    expect(screen.queryByText(/generated by/i)).toBeNull();
  });

  it("keeps the static generatedBy footer visible while loading", () => {
    render(
      <AiExplanation
        narrative=""
        provenance={[]}
    ...
    // Footer is static provenance metadata, not narrative content — it stays
    // visible alongside the skeleton (matches the mockup).
    expect(screen.getByText(/generated by sf-llm · explain/i)).toBeInTheDocument();
    expect(screen.getAllByTestId("ai-explanation-skeleton").length).toBeGreaterThan(0);
  });
});
